...
import { AdicionaItemInput, RemoveItemInput } from '../../../src/domain/entities/types/pedidoService.type';
...
describe('PedidoRepository', () => {
  ...
  it('Teste deve atualizar status do pedido', async () => {
    ...
    const pedidoAtualizado = await pedidoRepository.atualizaStatusDoPedido(pedido.id, statusDoPedido.AGUARDANDO_PAGAMENTO);
  ...
  it('Teste deve buscar pedido', async () => {
    ...
    const pedidoAtualizado = await pedidoRepository.retornaPedido(pedido.id, null);
    expect(pedidoAtualizado).toMatchObject(pedido);
  });
  ...
  it('Teste deve buscar pedido com clienteId', async () => {
    ...
    const pedidoAtualizado = await pedidoRepository.retornaPedido(pedido.id, "2");
    expect(pedidoAtualizado).toMatchObject(pedido);
  });
  ...
  it('Teste listar pedidos', async () => {
    ...
    const pedidoAtualizado = await pedidoRepository.listaPedidos([statusDoPedido.RASCUNHO], "2");
