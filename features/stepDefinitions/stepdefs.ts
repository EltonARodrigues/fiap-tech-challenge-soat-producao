...
Then(
  "deve retornar um pedido atualizado com os items adicionados",
  async function () {
    const respBody = await response.json();
...
Then(
  "o status da resposta ao adicionar um item deve ser {int}",
  function (status) {
    assert.equal(response.status, status);
  }
);
...
When("passar o id do item que deseja remover", async function () {
  response = await fetch(
    `${url_endpoint}/pedido/${pedidoId}/remover-item/${itemRemoverId}`,
    {
      method: "DELETE",
    }
  );
...
Then(
  "deve retornar um pedido atualizado o item escolhido removido",
  async function () {
    const respBody = await response.json();
    pedido = respBody.message;
...
Then(
  "o status da resposta ao remover um item deve ser {int}",
  function (status) {
    assert.equal(response.status, status);
  }
);
...
Given(
  "que exite um pedido criado e itens adicinados deve finalizar a customizacao",
  async function () {
    assert(pedidoId != null);
    assert.equal(pedido.status, statusDoPedido.RASCUNHO);
  }
...
Then(
  "deve retornar um pedido atualizado com o status Aguardando pagamento",
  async function () {
    const respBody = await response.json();
    pedido = respBody.message;
    assert.equal(
      respBody?.message?.status,
...
Then(
  "o status da resposta para aguardando pagamento deve retornar {int}",
  function (status) {
    assert.equal(response.status, status);
  }
);
...
When("passar o id do pedido que esta em Aguardando preparo", async function () {
  response = await fetch(
    `${url_endpoint}/pedido/iniciar-preparo/?pedidoId=${pedidoId}`,
    {
      method: "PATCH",
    }
  );
...
Then(
  "deve retornar um pedido atualizado com o status Em preparo",
  async function () {
    const respBody = await response.json();
    pedido = respBody.message;
...
Then(
  "o status da resposta para Em preparo deve retornar {int}",
  function (status) {
    assert.equal(response.status, status);
  }
);
...
When("passar o id do pedido que esta em Em preparo", async function () {
  response = await fetch(
    `${url_endpoint}/pedido/finalizar-preparo/${pedidoId}`,
    {
      method: "PATCH",
    }
  );
...
Then(
  "deve retornar um pedido atualizado com o status Pronto",
  async function () {
    const respBody = await response.json();
    pedido = respBody.message;
...
Then(
  "deve retornar um pedido atualizado com o status Entregue",
  async function () {
    const respBody = await response.json();
    pedido = respBody.message;
...
Then(
  "o status da resposta para Entregue deve retornar {int}",
  function (status) {
    assert.equal(response.status, status);
  }
);
...
Then(
  "deve retornar a lista de pedidos que bate com o status",
  async function () {
    const respBody = await response.json();
