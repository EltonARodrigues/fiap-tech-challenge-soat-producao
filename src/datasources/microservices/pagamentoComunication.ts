...
export default class MetodoPagamentoMicroserviceComunication
  implements MetodoPagamentoRepository
{
  async retornaMetodoPagamentoValido(metodoId: string): Promise<boolean> {
    try {
      ...
      if (response.status === 200) {
        ...
        return metodosPagamento?.message?.some(
          (metodo: MetodoPagamento) => metodo.id === metodoId
        );
      }
    } catch (err: unknown) {
      console.log(err);
